...
import SideNav from "../components/customers/SideNav";
...
export default {
  ...
  computed: {
    ...mapGetters("customerLogin", ["customer"]),
  },
  methods: {
    ...mapActions("customerLogin", ["unauthenticate"]),
    toggleSidebar() {
      this.show = !this.show;
    ...
    logout() {
      this.unauthenticate()
        .then(() => {
          this.$router.replace({ name: "customers.login" });
        })
        .catch((error) => {
